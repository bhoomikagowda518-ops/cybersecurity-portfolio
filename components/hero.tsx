    ...
    <div className="flex items-center gap-2.5">
      <a
        ...
        className="
          ...
          hover:bg-[var(--accent-cyan)]/[0.06]
          hover:text-[var(--accent-cyan)]
          hover:shadow-[0_6px_20px_rgba(34,211,238,0.08)]
        "
      >
        <GithubIcon />
      </a>
...
function HeroPhoto({ mobile = false }: { mobile?: boolean }) {
  return (
    ...
    >
      {/* Very subtle glow — kept tight to the photo */}
      <div
        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--accent-cyan)]/10 blur-[35px] animate-photo-glow ${
          mobile
      ...
      {/* Single thin technical ring */}
      <div
        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[var(--accent-cyan)]/20 animate-spin-slow ${
          mobile
            ? 'h-[270px] w-[270px] sm:h-[335px] sm:w-[335px]'
            : 'h-[300px] w-[300px] sm:h-[335px] sm:w-[335px]'
        }`}
      />

      {/* Photo movement */}
      <div className="relative animate-photo-float">
        {/* Tight subtle photo halo */}
        <div className="absolute -inset-[3px] rounded-full border border-[var(--accent-cyan)]/25" />

        <div
          className={`relative overflow-hidden rounded-full border-2 border-[var(--border-strong)] bg-[var(--panel)] shadow-[0_0_28px_rgba(34,211,238,0.10)] ${
            mobile
              ? 'h-64 w-64 sm:h-80 sm:w-80'
              : 'h-72 w-72 sm:h-80 sm:w-80'
          }`}
